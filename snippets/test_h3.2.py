...
def _get_prop_vs_prop_dss_score(treatment, con, dfs=None, use_percentage=None, use_labels=None, metric=None, as_percentage=None, is_categorical=None, alternative=None):
    ...
    df_prop_ref, df_resp_ref = get_prop_resp("t10a")
    ...
    if is_categorical:
        ...
        print("Conclusion: ", generate_cat_stat_sentence(np.mean(prop_values_ref), np.std(prop_values_ref), np.mean(prop_dss_values), np.std(prop_dss_values), s, p, dof, diff=diff, label1="t10.dss", label2=treatment+".dss"))
        ...
        p = res.results[3]
        r = res.results[9]
        print("Conclusion: ", generate_stat_sentence(np.mean(prop_values_ref), np.std(prop_values_ref), np.mean(prop_dss_values), np.std(prop_dss_values), s, p, dof, diff=diff, label1="t10.dss", label2=treatment+".dss"))
