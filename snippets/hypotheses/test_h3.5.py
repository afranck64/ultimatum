...
def _get_prop_vs_prop_dss_score(treatment, con, dfs=None, use_percentage=None, use_labels=None, metric=None, as_percentage=None, is_categorical=None, alternative=None):
    ...
    diff = None
    if is_categorical:
        table, res = rp.crosstab(pd.Series(metric_ref_values), pd.Series(metric_values), test='chi-square')
        s, p, r = res.results.values

        print("Conclusion: ", generate_cat_stat_sentence(np.mean(metric_ref_values), np.std(metric_ref_values), np.mean(metric_values), np.std(metric_values), s, p, dof, diff=diff, label1="t11a.dss", label2=treatment+".dss"))
        test_label = f"(pearson chi2)"
    else:

        #print("Ranksums", stats.ranksums(metric_ref_values, metric_values))

        table, res = rp.ttest(pd.Series(metric_ref_values), pd.Series(metric_values), paired=False)
        s = res.results[2]
        ...
        print("Conclusion: ", generate_stat_sentence(np.mean(metric_ref_values), np.std(metric_ref_values), np.mean(metric_values), np.std(metric_values), s, p, dof, diff=diff, label1="t11a.dss", label2=treatment+".dss"))
    ...
    if as_percentage:
        res = {
            ...
            "T11A ": f'{100 * metric_value_ref:.2f} %',
            "prop:dss - auto prop": f'{100 * (metric_value - metric_value_ref):.2f} %',
        }
    else:
        res = {
            ...
            "prop:dss - auto prop": f'{(metric_value - metric_value_ref):.2f} %',
        }
...
#     return _get_prop_vs_prop_dss_score(treatment, con, dfs, use_percentage, use_labels, metric=metrics.get_dss_usage, as_percentage=True, is_categorical=True)

@mark_for_stats()
def get_proposer_gain(treatment, con, dfs=None, use_percentage=None, use_labels=None):
    return _get_prop_vs_prop_dss_score(treatment, con, dfs, use_percentage, use_labels, metric=metrics.get_proposer_gain_df, as_percentage=False, is_categorical=False)

@mark_for_stats()
def get_proposer_gain_no_dss(treatment, con, dfs=None, use_percentage=None, use_labels=None):
    return _get_prop_vs_prop_dss_score(treatment, con, dfs, use_percentage, use_labels, metric=metrics.get_proposer_gain_no_dss_df, as_percentage=False, is_categorical=False)

@mark_for_stats()
def get_proposer_rel_gain(treatment, con, dfs=None, use_percentage=None, use_labels=None):
    return _get_prop_vs_prop_dss_score(treatment, con, dfs, use_percentage, use_labels, metric=metrics.get_rel_gain_proposer_df, as_percentage=False, is_categorical=False)
