...
class JobConfig(dict):
    def __init__(self, job_id, api_key, nb_rows=0, unique_worker=False):
        super().__init__()
        self["job_id"] = job_id
        self["api_key"] = api_key
        self["nb_rows"] = nb_rows
        self["unique_worker"] = False
...
def update_job(con, job_id, job_config, table="jobs"):
    if not table_exists(con, table):
        ...
        with con:
            ...
            if check:
                # apply an update
                con.execute(f"UPDATE {table} SET api_key=?", (job_config['api_key'],))
            else:
                con.execute(f"INSERT INTO {table} (job_id, api_key) VALUES (?, ?)", (job_id, job_config['api_key']))
...
def index():
    if request.method=="POST":
        ...
        job_config = JobConfig(job_id=job_id, api_key=api_key)
